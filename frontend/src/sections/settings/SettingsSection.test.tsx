...
describe('<SettingsSection /> · idioma de la interfaz', () => {
  ...
  it('en inglés: las pestañas se muestran traducidas y los códigos de pestaña no cambian', () => {
    ...
    expect(tabs.map((tab) => tab.textContent)).toEqual(['Company details', 'Channels and integrations', 'Billing and payments', 'Automations', 'Forms', 'Reopen initial setup'])
    expect(tabs.map((tab) => tab.id)).toEqual(['settings-tab-empresa', 'settings-tab-integrations', 'settings-tab-billing', 'settings-tab-automation', 'settings-tab-forms', 'settings-tab-reopen'])
  ...
  it('en español (por defecto): pestañas en español y cambio de pestaña a Canales e integraciones', async () => {
    ...
    expect(within(tablist).getAllByRole('tab').map((tab) => tab.textContent)).toEqual(['Datos de empresa', 'Canales e integraciones', 'Facturación y pagos', 'Automatizaciones', 'Formularios', 'Reabrir configuración inicial'])
