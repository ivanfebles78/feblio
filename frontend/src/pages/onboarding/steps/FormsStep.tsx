  ...
  return (
    <div className="space-y-8">
      <p className="rounded-xl bg-slate-50 px-3 py-2 text-xs text-slate-600">
        Los formularios amplían el sistema actual de enlaces para clientes: cada enlace generado desde Plantillas → Formulario de clientes puede usar una de estas plantillas.
      </p>
      ...
      <section aria-labelledby="sec-forms-list">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button type="button" onClick={() => setEditing(emptyTemplate())} className="btn-primary !px-3 !py-2 text-xs">
            <Plus className="h-3.5 w-3.5" aria-hidden="true" /> Crear formulario
          </button>
        </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {t.is_default && (
                      <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-semibold text-amber-700 ring-1 ring-amber-200">
                        <Star className="h-3 w-3" fill="currentColor" aria-hidden="true" /> Predeterminado
                      </span>
                    )}
                    {!t.is_active && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-500">Inactivo</span>}
                  </div>
                  <p className="text-xs text-slate-500">
                    {t.fields.length} campos · {t.required_documents.length} documentos · caduca a los {t.link_expiry_days} días
                  </p>
                </div>
                <div className="flex shrink-0 gap-1.5">
                  <button type="button" onClick={() => ctx.runAction(() => setDefaultFormTemplate(empresaId, t.id))} disabled={t.is_default} className={`grid h-8 w-8 place-items-center rounded-lg border ${t.is_default ? 'border-amber-200 bg-amber-50 text-amber-500' : 'border-slate-200 text-slate-300 hover:text-amber-400'}`} aria-label={t.is_default ? `${t.name} es el predeterminado` : `Marcar ${t.name} como predeterminado`}>
                    <Star className="h-4 w-4" fill={t.is_default ? 'currentColor' : 'none'} aria-hidden="true" />
                  </button>
                  <button type="button" onClick={() => setPreview(preview?.id === t.id ? null : t)} className="inline-flex items-center gap-1 rounded-lg border border-brand-200 bg-brand-50 px-2.5 py-1.5 text-xs font-semibold text-brand-700 hover:bg-brand-100" aria-pressed={preview?.id === t.id}>
                    <Eye className="h-3.5 w-3.5" aria-hidden="true" /> Vista previa
                  </button>
                  <button type="button" onClick={() => setEditing({ ...t })} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">
                    <Pencil className="h-3.5 w-3.5" aria-hidden="true" /> Editar
                  </button>
                  <button type="button" onClick={() => setConfirmDelete(t)} disabled={t.is_default} className="rounded-lg border border-slate-200 px-2 py-1.5 text-slate-400 hover:bg-red-50 hover:text-red-500 disabled:opacity-40" aria-label={`Eliminar ${t.name}`}>
                    <Trash2 className="h-4 w-4" aria-hidden="true" />
                  </button>
                </div>
              </div>
              {editing?.id === t.id && <div className="mt-3"><FormTemplateEditor initial={editing} busy={busy} onSave={save} onCancel={() => setEditing(null)} /></div>}
              {preview?.id === t.id && <div className="mt-3"><FormPreview template={t} empresaName={ctx.snapshot?.empresa.name ?? ''} projectTypes={ctx.snapshot?.empresa.intake_config?.project_types ?? []} /></div>}
            </li>
          ))}
          {templates.length === 0 && <li className="p-6 text-center text-sm text-slate-400">Aún no hay formularios. Crea el primero.</li>}
        </ul>
      </section>
      ...
      <section aria-labelledby="sec-forms-map">
        ...
        <p className="mb-3 text-xs text-slate-500">Qué formulario se envía desde cada canal y con qué mensaje.</p>
        <ChannelFormMapping rules={rules} templates={templates} onChange={updateRule} />
      </section>

      <ConfirmDialog open={confirmDelete !== null} title={`¿Eliminar «${confirmDelete?.name}»?`} tone="danger" confirmLabel="Eliminar" onConfirm={remove} onCancel={() => setConfirmDelete(null)}>
        Los enlaces ya generados con esta plantilla seguirán funcionando con el formulario básico.
      </ConfirmDialog>
    </div>
  )
}
