...
1. **Antes de producción, prueba en una rama** (Supabase → Branches → New branch) o en un proyecto de desarrollo:
   - SQL Editor → `0009_onboarding_wizard.sql`, `0010_sync_profile_email.sql` y `0011_security_hardening.sql` → Run (en ese orden).
   - SQL Editor → `database/tests/0009_rls_isolation.sql`, `0010_sync_profile_email.sql`, `0011_security_audit.sql` y `0011_signup_roles.sql` → Run (terminan en `rollback`; deben imprimir «… han pasado»).
...
2. Aplica `0009`, `0010` y `0011` en producción cuando la rama esté verificada. Es idempotente y no destruye datos: las empresas
   existentes quedan con `onboarding_status = 'completed'` (pueden reabrir el asistente desde Configuración).
