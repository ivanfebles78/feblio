...
begin
  ...
  insert into pg_temp._t values ('R1a empresa: profiles.role = empresa', r.role = 'empresa');
  insert into pg_temp._t values ('R1b empresa: fila en empresas con datos del registro', r.name = 'Nueva Empresa SL' and r.cif = 'B12345674' and r.entity_type = 'company');
  insert into pg_temp._t values ('R1c empresa: profiles.empresa_id enlazado y propietario del onboarding', r.empresa_id is not null and r.is_onboarding_owner);
  insert into pg_temp._t values ('R1d empresa: arranca verificación (email_verified=false) y wizard (not_started)', r.email_verified = false and r.onboarding_status = 'not_started');
  ...
  insert into pg_temp._t values ('R1e empresa: consentimientos registrados', n = 2);
  select count(*) into n from public.audit_events where user_id = v_emp and action = 'empresa.registered';
  insert into pg_temp._t values ('R1f empresa: auditoría empresa.registered', n = 1);

  -- R2: intento de admin
  select role::text, empresa_id into v_text, v_other_e from public.profiles where id = v_adm;
  insert into pg_temp._t values (case when v_simulated then 'R2a admin manipulado → cliente' else 'R2a admin manipulado → cliente (sin simular supabase_auth_admin)' end, v_text = 'cliente');
  insert into pg_temp._t values ('R2b admin manipulado: sin empresa creada', (select empresa_id from public.profiles where id = v_adm) is null);
  ...
  insert into pg_temp._t values ('R2c admin manipulado: auditado como bloqueado', n = 1);
  ...
  insert into pg_temp._t values ('R2d admin manipulado: is_admin() = false', public.is_admin() = false);
  select count(*) into n from public.empresas;
  insert into pg_temp._t values ('R2e admin manipulado: no lee empresas de la plataforma', n = 0);
  begin perform public.get_onboarding(); v_ok := false; exception when others then v_ok := true; end;
  insert into pg_temp._t values ('R2f admin manipulado: no accede al onboarding empresarial', v_ok);
  reset role;

  -- R3: cliente válido
  select role::text, empresa_id into v_text, v_other_e from public.profiles where id = v_cli;
  insert into pg_temp._t values ('R3a cliente: profiles.role = cliente', v_text = 'cliente');
  insert into pg_temp._t values ('R3b cliente: no crea empresa', (select empresa_id from public.profiles where id = v_cli) is null and not exists (select 1 from public.empresas where name = 'Casa Chona'));
  ...
  insert into pg_temp._t values (case when session_user in ('postgres') or v_simulated then 'R4 seed con opt-in: crea admin' else 'R4 seed con opt-in (sesión no administrativa): degradado' end,
  ...
  insert into pg_temp._t values ('R5 seed sin opt-in: no crea admin', v_text = 'cliente');
end $$;
