...
// Cualquier fallo de autorización responde 404 «Enlace no válido» (mensaje único). Límite de
// frecuencia por IP y por acceso en la propia RPC (429). No se registra el token.
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'
import { NEUTRAL_ERROR, outcomeFromRpcError, parseDownloadRequest, rateKeyFromHeaders, safeDownloadName, SIGNED_URL_TTL_SECONDS } from '../_shared/solicitudes/descarga.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return json({ error: NEUTRAL_ERROR }, 400)
  }
  const parsed = parseDownloadRequest(body)
  if (!parsed) return json({ error: NEUTRAL_ERROR }, 400)
  ...
  if (error || !data || typeof data !== 'object') {
    const out = outcomeFromRpcError(error?.message)
    return json(out.body, out.status)
  }
  const doc = data as { storage_path?: unknown; original_name?: unknown }
  if (typeof doc.storage_path !== 'string') return json({ error: NEUTRAL_ERROR }, 404)
  ...
  if (signed.error || !signed.data?.signedUrl) {
    ...
    return json({ error: NEUTRAL_ERROR }, 404)
  }
